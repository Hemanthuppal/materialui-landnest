...
const EditRentForm = () => {
   ...
   const handleSubmit = async (e) => {
    ...
    // Merge form data with additional location info
    ...
    // Append each image file from workPhotos
    if (workPhotos && workPhotos.length > 0) {
        workPhotos.forEach((file, index) => {
            // Ensure you're matching backend expectations with field name
            formDataToSend.append('new_property_images[]', file); // Note the [] for arrays
            console.log(`Photo ${index + 1} appended:`, file.name);
        });
    } else {
        console.log('No new work photos to upload.');
    }

    console.log('Final workPhotos before sending:', workPhotos);
