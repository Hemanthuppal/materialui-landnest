...
import BottomNavbar from './CustomNavbarHotProperties';
import { AuthContext } from '../AuthContext/AuthContext';
import { BASE_URL } from '../Api/ApiUrls';

const HotProperty_Saves = () => {
        ...
        const fetchData = async () => {
          try {
            ...
            const savedProperties = cartItems
              .map(cartItem => {
                ...
                const property = propertiesResponse.data.find(
                  p => p.property_id === cartItem.property_id && 
                       p.type && 
                       p.type.toLowerCase().includes("best-deal")
                );
              ...
              .filter(Boolean)
              .sort((a, b) => new Date(b.created_at) - new Date(a.created_at));// Remove any null entries
   ...
   const filteredProperties = saved.filter((property) => {
    ...
    return (
      <Box sx={{ backgroundColor: 'rgb(239, 231, 221)', minHeight: '110vh' }}>
        ...
        {/* Property Types */}
                    <Box sx={{ px: 2 }}>
                      <Typography variant="subtitle1" sx={{ mb: 1 }}>Saved Hot Properties </Typography>
                      <Box
                        sx={{
                          display: 'flex',
                          gap: 1,
...
export default HotProperty_Saves;