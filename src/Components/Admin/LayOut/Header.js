...
export default function Header() {
  ...
  const drawer = (
    <Box sx={{ 
      width: 280,
      background: 'linear-gradient(145deg, rgba(232,224,208,0.95), rgba(216,204,186,0.95))',
      height: '100%'
    }}>
      <Box sx={{ 
        display: 'flex', 
        ...
        {navItems.map((item) => (
          <ListItem key={item.label} disablePadding>
            <ListItemButton
              ...
              sx={{
                '&.Mui-selected': {
                  background: 'linear-gradient(135deg, rgba(241,229,124,0.3), rgba(230,206,69,0.3))',
                  borderLeft: '4px solid #e6ce45'
                },
                '&:hover': {
                  background: 'linear-gradient(135deg, rgba(241,229,124,0.2), rgba(230,206,69,0.2))'
                }
              }}
            >
              <ListItemText
      ...
      >
        <Toolbar>
          {isMobile ? (
            ...
            <>
              <Link to="/view" style={{ textDecoration: 'none', display: 'flex', alignItems: 'center' }}>
                <img 
                  src={Logo} 
                  alt="logo" 
                  style={{ 
              ...
              <Box sx={{ 
                ...
                {navItems.map((item) => (
                  ...
                  sx={{
                    ...
                    minWidth: 'auto',
                    px: 2,
                    borderBottom: isActive(item.path) ? '3px solid #f7e896' : '3px solid transparent',
                    borderRadius: 0,
                    '&:hover': {
                      color: '#f7e896',
                      background: 'rgba(247,232,150,0.1)'
