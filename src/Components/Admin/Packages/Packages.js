...
const ConstructionPackages = () => {
  ...
  const extractRupeeValues = (content) => {
    const rupeeRegex = /₹(\d{1,3}(?:,\d{3})*(?:\.\d{2})?|\d+)/g;
  ...
  const handleSave = (pkgId, sectionId) => {
    setEditingField(null);
  };

  const handleRupeeValueChange = (pkgId, sectionId, content, index, length, newValue) => {
    const newContent = content.substring(0, index) + newValue + content.substring(index + length);
    setEditableContent((prev) => ({
      ...prev,
  ...
  const renderContentWithEditableRupeeValues = (pkgId, sectionId, content) => {
    ...
    rupeeValues.forEach((rupee, i) => {
      // Add text before the rupee value
      ...
      // Add editable rupee value
      elements.push(
        <TextField
          key={`edit-${i}`}
          value={rupee.value}
          onChange={(e) => handleRupeeValueChange(
            pkgId, 
            sectionId, 
            editableContent[`${pkgId}-${sectionId}`], 
            rupee.index, 
            rupee.length, 
            e.target.value
          )}
    ...
    // Add remaining text after last rupee value
  ...
  const sectionContent = {
    package2000: {
      ...
      fabrication:
        "• <strong>MS Staircase Railing:</strong> ₹450 per sq. ft. (3′ height)\n• <strong>MS Standard Gate:</strong> ₹450 per sq. ft. (5′ height)",
      plumbing:
      ...
      sections: [
        {
          id: "pkg2000One",
          title: "Design & Drawings",
          content: sectionContent.package2000.design,
        },
        {
          id: "pkg2000Two",
          title: "Core Construction Materials",
          content: sectionContent.package2000.materials,
        },
        {
          id: "pkg2000Three",
          title: "Flooring & Wall Tiling",
          content: sectionContent.package2000.flooring,
        },
        {
          id: "pkg2000Four",
          title: "Windows",
          content: sectionContent.package2000.windows,
        },
        {
          id: "pkg2000Five",
          title: "Doors",
          content: sectionContent.package2000.doors,
        },
        {
          id: "pkg2000Six",
          title: "Painting & Finishing",
          content: sectionContent.package2000.painting,
        },
        {
          id: "pkg2000Seven",
          title: "Fabrication Works",
          content: sectionContent.package2000.fabrication,
        },
        {
          id: "pkg2000Eight",
          title: "Plumbing & Sanitary",
          content: sectionContent.package2000.plumbing,
        },
        {
          id: "pkg2000Nine",
          title: "Extra Charges",
          content: sectionContent.package2000.extra,
        },
        {
          id: "pkg2000Ten",
          title: "Government Sanctions & Electrical Assistance",
          content: sectionContent.package2000.sanctions,
        },
        {
          id: "pkg2000Eleven",
          title: "Elevation Budget",
          content: sectionContent.package2000.elevation,
        },
        {
          id: "pkg2000Twelve",
          title: "Site Audit & Reporting",
          content: sectionContent.package2000.audit,
        },
      ],
    },
    {
      ...
      sections: [
        ...
        {
          id: "pkg2300Ten",
          title: "Exclusions & Additional Charges",
          content: sectionContent.package2300.extra,
        },
        {
          id: "pkg2300Eleven",
          title: "Government Approvals & Utilities",
          ...
          {packages.map((pkg, index) => (
            <Grow in={true} timeout={index * 200 + 400} key={pkg.id}>
              <Card sx={{ borderRadius: "16px", boxShadow: "0 10px 30px rgba(0,0,0,0.1)", overflow: "hidden", transition: "transform 0.4s, box-shadow 0.4s", "&:hover": { transform: "translateY(-8px)", boxShadow: "0 15px 35px rgba(0,0,0,0.15)" }, border: "none" }}>
                <CardHeader
                  title={<Typography variant="h5" sx={{ fontWeight: 700, color: "white", fontSize: "1.6rem", letterSpacing: "0.5px" }}>{pkg.title}</Typography>}
                  subheader={<Typography variant="h4" sx={{ fontWeight: 700, color: "white", mt: 1.5, background: "rgba(255,255,255,0.2)", display: "inline-block", px: 3, py: 1, borderRadius: "20px", backdropFilter: "blur(5px)", fontSize: "1.5rem" }}>{pkg.price}</Typography>}
                  sx={{ background: pkg.gradient, padding: "30px 20px", textAlign: "center", position: "relative", "&:after": { content: '""', position: "absolute", bottom: 0, left: "50%", transform: "translateX(-50%)", width: "100px", height: "4px", backgroundColor: "rgba(255,255,255,0.5)", borderRadius: "2px" }}}
                />
                  ...
                  {pkg.sections.map(( section) => (
                    <Accordion
                      ...
                      sx={{
                        ...
                        "&:hover": { backgroundColor: "rgba(74, 0, 224, 0.03)" },
                      }}
                    >
                      <AccordionSummary
                        expandIcon={expanded[pkg.id] === section.id ? <RemoveIcon sx={{ color: pkg.color }} /> : <AddIcon sx={{ color: "#666" }} />}
                        aria-controls={`${section.id}-content`}
                        id={`${section.id}-header`}
                        sx={{ padding: "0 25px", minHeight: "68px !important", "& .MuiAccordionSummary-content": { margin: "12px 0", alignItems: "center" } }}
                      >
                        <Typography sx={{ fontWeight: 600, color: expanded[pkg.id] === section.id ? pkg.color : "#444", fontSize: "1.2rem", letterSpacing: "0.2px" }}>
                          {section.title}
                        </Typography>
                        <IconButton onClick={() => handleEdit(pkg.id, section.id, section.content)} sx={{ marginLeft: "auto" }}>
                          <EditIcon />
                        </IconButton>
                      </AccordionSummary>
                      <AccordionDetails
                      ...
                      >
                        ...
                        >
                          {renderContentWithEditableRupeeValues(
                            pkg.id, 
                            section.id, 
                            editableContent[`${pkg.id}-${section.id}`] || section.content
                          )}
                        </Typography>
                        {editingField === `${pkg.id}-${section.id}` && (
                          <Button
                            variant="contained"
                            color="primary"
                            onClick={() => handleSave(pkg.id, section.id)}
                            sx={{ marginTop: 2 }}
                            startIcon={<SaveIcon />}
                          >
                            Save Changes
