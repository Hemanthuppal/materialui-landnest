  ...
  const handleSave = async (pkgId, sectionId) => {
    try {
      const content =
        ...
        sectionContent.package2300[
          sectionId.replace("pkg2300", "").toLowerCase()
        ];
      ...
      let updatePayload = {};
      const sectionKey = sectionId.replace("pkg2300", "").toLowerCase();
      ...
      if (Object.keys(filteredPayload).length > 0) {
        await axios.put(`https://landnest.net:81/packages/4/`, filteredPayload);
